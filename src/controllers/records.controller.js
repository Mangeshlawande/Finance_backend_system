...
export const fetchAllRecords = asyncHandler(async (req, res) => {
    ...
    const { data, total, page, limit } = await getAllRecords({
        userId: req.user.id,    // scoped to requesting user
        ...parsed.data,
    });
...
export const fetchRecordById = asyncHandler(async (req, res) => {
    ...
    const record = await getRecordById(parsed.data.id, req.user.id);   // ownership enforced
...
export const updateRecordById = asyncHandler(async (req, res) => {
    ...
    const record = await updateRecord(idParsed.data.id, req.user.id, bodyParsed.data);  // ownership enforced
...
export const deleteRecordById = asyncHandler(async (req, res) => {
    ...
    await softDeleteRecord(parsed.data.id, req.user.id);    // ownership enforced
